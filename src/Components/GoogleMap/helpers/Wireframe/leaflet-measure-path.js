...
import { getCenter } from "../leaflet/helper";
...
L.Polyline.include({
    ...
    showMeasurementsLine: function (options) {
        if (!this._map || this._measurementLayer) return this;
        ...
        this._measurementLayer = L.layerGroup().addTo(this._map);
        this.updateMeasurementsLIne();

        this._map.on('zoomend', this.updateMeasurementsLIne, this);

        return this;
    },
    updateMeasurementsLIne: function () {
        ...
        let perimeter = 0
        if (this._measurementOptions.showDistances && latLngs.length > 1 && !isPolygon ) {
        ...
        if (isPolygon && options.showArea && latLngs.length > 2) {
            ...
            const center = getCenter(this, this._map);

            L.marker.measurement({ lat: center.lat + d, lng: center.lng },
                options.refId, 'areaId', 0, options)
                .addTo(this._measurementLayer);
            L.marker.measurement(center,
                formatter(area, this), 'areaS', 0, options)
                .addTo(this._measurementLayer);
            L.marker.measurement({ lat: center.lat - 0.000007, lng: center.lng },
                (perimeter * 3.2808399).toFixed(1), 'areaPerimeter', 0, options)
                .addTo(this._measurementLayer);
        }
    ...
    updateMeasurements: function () {
        ...
        if (isPolygon && options.showArea && latLngs.length > 2) {
            ...
            L.marker.measurement(this.getBounds().getCenter(),
                formatter(area, this), options.lang.totalArea, 0, options)
                .addTo(this._measurementLayer);
        }
